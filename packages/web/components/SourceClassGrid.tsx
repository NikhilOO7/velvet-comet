    ...
    <div className="flex flex-wrap gap-2">
      {ALL.map((cls) => {
        ...
        return (
          <div
            key={cls}
            className={`flex items-center gap-2 whitespace-nowrap rounded-lg border px-2.5 py-1.5 text-xs ${
              on ? 'border-accent/30 bg-accent/5 text-ink' : 'border-line/50 text-faint'
            }`}
          >
            <span
              className={`h-2 w-2 shrink-0 rounded-full ${on ? 'bg-accent shadow-glow' : 'bg-line'}`}
              aria-hidden
            />
