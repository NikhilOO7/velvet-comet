...
[PLAN.md](PLAN.md); the project write-up is [ONE-PAGER.md](ONE-PAGER.md).
...
The project started from 11 pieces of real customer feedback. The loudest and
...
ask **already ships** ("know the product before you build"),
...
Crucially, **narrow and deep beats wide and shallow**, so this
solves *one* customer's problem end-to-end rather than touching all eleven.

## 3. How the completeness engine works

The core is a bounded, *saturating* fan-out that changes the **queries**, not the
result count:
...
- **Per-step action debugging & self-maintaining extractors**: strong gaps, but
  each is its own deep product. Bundling them would be wide-and-shallow.
...
- **Code (caught by a failing test):** an AI-written test fixture generated
  ...
  *fixture* was wrong; I fixed the generator, not the test. Exactly why the engine
...
| [ONE-PAGER.md](ONE-PAGER.md) | The project write-up. |
